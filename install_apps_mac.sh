...
install_apps::dev_stuff() {
    ...
    brew install docker              I          # one should probably use docker for all dev stuff and run nothing on local machine
    brew install virtualbox                     # even though everything is containered in 202X one might need VM's
    # Extra editors
    ...
    # extra utils
    brew install coreutils findutils gnu-tar    # gnu versions for many cases are just better
    brew install gnu-sed gawk gnutls gnu-indent # gnu versions for many cases are just better
    brew install gnu-getopt grep wget gzip      # gnu versions for many cases are just better
